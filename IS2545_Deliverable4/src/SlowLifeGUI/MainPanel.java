...
public class MainPanel extends JPanel {
    ...
    /*Modified Method*/
    private int convertToIntModified(int x){
    	return x;
    }
    ...
    /*Modified Method*/
    private int getNumNeighborsModified(int x, int y) {
	int size = _size;
	int leftX = (x - 1) % size;
	int rightX = (x + 1) % size;
	int upY = (y - 1) % size;
	...
	return convertToIntModified(numNeighbors);
    }
    ...
    /*Modified Method*/
    public void runContinuousModified(){
    	_running = true;
    	while(_running){
    		System.out.println("Running...");
    		try{
